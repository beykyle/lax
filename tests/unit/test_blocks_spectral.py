...
def test_block_interpolators_match_per_block_interpolants() -> None:
    block_groups = partial_wave_groups((0, 1))
    solver = _compile(blocks=block_groups, solvers=("rmatrix_direct",))
    kernel = gaussian_kernel(20.0)
    assert solver.phases_direct is not None
    samples = solver.phases_direct(solver.nonlocal_potential(kernel))  # (N_b, N_E, 1)
    assert solver.interpolate_phases is not None
    interpolant = solver.interpolate_phases(samples)

    # At a grid knot the Padé interpolant reproduces the per-block samples.
    knot = float(ENERGIES[2])
    np.testing.assert_allclose(
        np.asarray(interpolant(knot)), np.asarray(samples[:, 2]), rtol=1e-8, atol=1e-10
    )

    # And per-block it equals the per-block channels-compiled interpolant.
    for b, group in enumerate(block_groups):
        single = _compile(channels=group, solvers=("rmatrix_direct",))
        assert single.phases_direct is not None
        assert single.interpolate_phases is not None
        single_samples = single.phases_direct(single.nonlocal_potential(kernel))
        single_interpolant = single.interpolate_phases(single_samples)
        query = 17.3
        np.testing.assert_allclose(
            np.asarray(interpolant(query)[b]),
            np.asarray(single_interpolant(query)),
            rtol=1e-8,
            atol=1e-10,
        )


def test_block_interpolator_rejects_wrong_leading_axis() -> None:
    block_groups = partial_wave_groups((0, 1))
    solver = _compile(blocks=block_groups, solvers=("rmatrix_direct",))
    assert solver.interpolate_phases is not None
    with pytest.raises(ValueError, match="block-batched samples"):
        solver.interpolate_phases(jnp.zeros((len(ENERGIES), 1)))
